...
class DiagnosticCoordinator:
    ...
    async def run_full_diagnostic_pipeline(
        self, text: str, onto_labels: List[str], onto_vectors: List[List[float]]
    ) -> DiagnosticResponseDTO:
        """
        ...
        try:
            ...
            if embeddings_result.is_failure:
                logger.error(
                    f"Failed to generate embeddings: {embeddings_result.error}"
                )
                return self._empty_response(text)
            ...
            mapped = self.semantic_matcher.filter_matched_symptoms(
                matching_result.value
            )

            logger.debug(
                f"Mapped symptoms | present={mapped.present} absent={mapped.absent}"
            )

            if not mapped.present:
                logger.warning(
                    "No ontology symptoms mapped above confidence threshold."
                )
                return self._empty_response(text)
            ...
            logger.debug(
                f"Neo4j inference returned {len(raw_records)} candidate diseases."
            )

            # Step 3.2: Calculate disease scores and coverage
            scoring_result = self.scoring_engine.evaluate(
                raw_records=raw_records, total_input_symptoms=len(mapped.present)
            )
            ...
            explanation_result = await self.xai_explainer.generate_explanation(
                diagnosis_result
            )

            if explanation_result.is_failure:
                logger.warning(
                    f"XAI explanation failed: {explanation_result.error}. Returning fallback."
                )
                explanation = XAIExplanationResult.fallback(
            ...
            return self._error_response(
                text, "Došlo je do interne greške pri obradi podataka."
            )
    ...
    def _empty_response(self, text: str) -> DiagnosticResponseDTO:
        ...
        return DiagnosticResponseDTO(
            ...
            absent_symptoms=[],
            explanation=XAIExplanationResult.fallback(
                "Nijedan simptom nije prepoznat u tekstu. Molimo unesite detaljniji opis simptoma."
            ),
        )
